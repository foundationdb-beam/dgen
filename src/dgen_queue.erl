...
push_k(?IS_DB(Db, Dir), Key, Items) ->
    erlfdb:transactional(Db, fun(Tx) -> push_k({Tx, Dir}, Key, Items) end);
push_k(?IS_TX(Tx, Dir), Key, Items) ->
...
consume_k(?IS_DB(Db, Dir), K, Key) ->
    erlfdb:transactional(Db, fun(Tx) -> consume_k({Tx, Dir}, K, Key) end);
consume_k(?IS_TD = Td, K, Key) ->
    case pop_k(Td, K, Key) of
        {ok, Vals} ->
            {Vals, undefined};
        {{error, empty}, Vals} ->
            {Vals, watch_push(Td, Key)}
    end.
...
length(?IS_DB(Db, Dir), Key) ->
    erlfdb:transactional(Db, fun(Tx) -> length({Tx, Dir}, Key) end);
length(?IS_TX(Tx, Dir), Key) ->
    QueueKey = get_queue_key(Key),
    PushKey = get_push_key(QueueKey),
...
watch_push(?IS_DB(Db, Dir), Key) ->
    erlfdb:transactional(Db, fun(Tx) -> watch_push({Tx, Dir}, Key) end);
watch_push(?IS_TX(Tx, Dir), Key) ->
    QueueKey = get_queue_key(Key),
    PushKey = get_push_key(QueueKey),
    erlfdb:watch(Tx, erlfdb_directory:pack(Dir, PushKey)).

pop_k(?IS_DB(Db, Dir), K, Key) ->
    erlfdb:transactional(Db, fun(Tx) -> pop_k({Tx, Dir}, K, Key) end);
pop_k(?IS_TX(Tx, Dir), K, Key) ->
    QueueKey = get_queue_key(Key),
    ItemKey = get_item_key(QueueKey),
